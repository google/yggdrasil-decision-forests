...
:: TF_SUPPORT=1 add support for TF.Record of TF.Example datasets and relies of
:: TensorFlow for all file IO primitives. If you don't need this dataset
:: format, TF_SUPPORT=0 is recommanded.
set TF_SUPPORT=0

:: Bazel is compatible with Visual Studio 2017 and 2019.
:: https://bazel.build/configure/windows#using
:: If you have multiple or other version of VS (e.g., VS2022), set "BAZEL_VC" accordingly.
...
IF %TF_SUPPORT%==0 (
  ...
  :: Support TensorFlow dataset, but does not use TensorFlow for file IO.
  set FLAGS=--config=windows_cpp17
  %BAZEL% build %FLAGS% //yggdrasil_decision_forests/cli/...:all || goto :error
  %BAZEL% test %FLAGS% //yggdrasil_decision_forests/{cli,metric,model,serving,utils}/...:all //examples:beginner_cc || goto :error

  :: Support TensorFlow dataset and use TensorFlow for file IO.
