...
This API allows to run Yggdrasil Decision Forests models trained with
Tensorflow.
...
import collections
import os
from typing import Any, Dict, List, Optional, Text, Iterator, Tuple
import uuid
...
def _detect_model_file_prefix(model_path: str) -> str:
  ...
  for dir_entry in tf.io.gfile.listdir(model_path):
    if dir_entry.endswith("done") and not tf.io.gfile.isdir(
        os.path.join(model_path, dir_entry)
    ):
      prefixes.append(dir_entry[: -len("done")])
  if not prefixes:
    raise ValueError(
        f'The model at {model_path} is invalid as it is missing a "done" file.'
    )
  if len(prefixes) > 1:
    raise ValueError(
        f"The model at {model_path} contains multiple YDF "
...
def _categorical_vocab_iterator(
...
class ModelV2(AutoTrackable):
  ...
  def __init__(
    ...
    if file_prefix is None:
      file_prefix = _detect_model_file_prefix(model_path)
...
class _InferenceArgsBuilder(AutoTrackable):
  ...
  def _register_input_feature(
    ...
    """
    assert self._feature_name_to_idx is not None
  ...
  def _create_str_to_int_tables(self):
    ...
    assert self._data_spec is not None
    assert self._feature_name_to_idx is not None
    ...
    for feature_idx in self._header.input_features:
      ...
      if (
        ...
        if self._force_string_to_unicode_conversion:
          vocabulary = [
              (key, item.index)
              for key, item in _categorical_vocab_iterator(
...
class _DiskModelLoader(_AbstractModelLoader, AutoTrackable):
  ...
  def get_model_path(self) -> Tensor:
    ...
    assert self._done_file is not None
