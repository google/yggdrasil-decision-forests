...
from ydf.deep import dataset as dataset_lib


class DatasetTest(parameterized.TestCase):

  def test_get_num_examples(self):
    self.assertEqual(dataset_lib.get_num_examples({"a": np.array([1, 2])}), 2)

  def test_batch_numpy_to_jax(self):
    result = dataset_lib.batch_numpy_to_jax({"a": np.array([1, 2])})
