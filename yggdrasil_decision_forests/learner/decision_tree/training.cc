...
// categorical-set condition with mask equal to {item}.
template <bool weighted>
void InitializeClassificationAttributeDistributions(
    const absl::Span<const UnsignedExampleIdx> selected_examples,
  ...
  const int num_attribute_classes = examples_by_candidate.size();
  if (attribute_distributions->size() < num_attribute_classes) {
    attribute_distributions->resize(num_attribute_classes);
  }
  for (const int candidate_attr_value : candidate_attributes_list) {
    auto& dist = (*attribute_distributions)[candidate_attr_value];
    dist.SetNumClassesIntDim(label_distribution.NumClasses());
    dist.Clear();
    *dist.mutable_neg() = label_distribution;
  ...
  *split_label_distribution.mutable_neg() = label_distribution;
  ...
  std::vector<utils::BinaryToIntegerConfusionMatrixDouble>&
      per_attribute_value_distributions =
          cache->catset_attribute_distributions_classification;
  ...
  std::vector<utils::IntegerDistributionDouble>&
      stats_examples_containing_attr_value =
          cache->catset_stats_examples_containing_attr_value_classification;
  if (stats_examples_containing_attr_value.size() < num_attribute_classes) {
    stats_examples_containing_attr_value.resize(num_attribute_classes);
  }
  for (int i = 0; i < num_attribute_classes; ++i) {
    stats_examples_containing_attr_value[i].SetNumClasses(
        label_distribution.NumClasses());
    stats_examples_containing_attr_value[i].Clear();
  }
  ...
  while (true) {
    ...
    // iteration.
    utils::IntegerDistributionDouble moved_example_stats_total(
        label_distribution.NumClasses());
    ...
    if (moved_example_stats_total.NumObservations() > 0) {
      ...
      for (const int attr_idx : candidate_attributes_list) {
        ...
        // Containing attr_idx.
        utils::IntegerDistributionDouble net_update = moved_example_stats_total;
        ...
        stats_examples_containing_attr_value[attr_idx].Clear();
      }
    }
...
FindSplitLabelRegressionFeatureCategoricalSetGreedyForward(
  ...
  *split_label_distribution.mutable_neg() = label_distribution;
  ...
  std::vector<utils::NormalDistributionDouble>&
      stats_examples_containing_attr_value =
          cache->catset_stats_examples_containing_attr_value_regression;
