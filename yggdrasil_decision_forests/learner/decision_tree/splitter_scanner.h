...
struct DoublePayload {
  // Resets payload to zero; useful for recovering the original feature.
  // The lowest 29 bits of a double created from a float are 0, so we can
  // safely use the lowest 29 bits without any precision loss.
  static HWY_INLINE double Clear(double d) {
    const uint64_t binary64 = hwy::BitCastScalar<uint64_t>(d);
    return hwy::BitCastScalar<double>(binary64 & ~0x1FFFFFFFULL);
  }

  // Sets payload to `bits` (up to 29 bits).
  static HWY_INLINE double Set(double d, uint32_t bits) {
    const uint64_t binary64 = hwy::BitCastScalar<uint64_t>(Clear(d));
    return hwy::BitCastScalar<double>(binary64 | (bits & 0x1FFFFFFF));
  }

  // Obtains the payload previously set by `Set`.
  static HWY_INLINE uint32_t Get(double d) {
    return static_cast<uint32_t>(hwy::BitCastScalar<uint64_t>(d) &
                                 0x1FFFFFFFULL);
  }
};
...
struct FlatBucketTraits<ExampleBucketType,
                        std::enable_if_t<std::is_same_v<
                            ...
                            LabelUnweightedCategoricalOneValueBucket>>> {
  using HWY_T = double;
  // Sorted with Key: Feature (double). Value: Label (int, in lowest 29 bits).
  static constexpr bool kIsSupported = true;
  static HWY_T* GetBuffer(PerThreadCacheV2* cache) {
    return cache->hwy_double_buffer.data();
  }

  static void Pack(HWY_T& hwy, const ExampleBucketType& bucket) {
    hwy = internal::DoublePayload::Set(
        static_cast<double>(bucket.feature.value),
        static_cast<uint32_t>(bucket.label.content.value));
  }
  static void Unpack(const HWY_T& hwy, ExampleBucketType& bucket) {
    bucket.feature.value =
        static_cast<float>(internal::DoublePayload::Clear(hwy));
    bucket.label.content.value =
        static_cast<int32_t>(internal::DoublePayload::Get(hwy));
  }
};
...
struct FlatBucketTraits<
    ...
    std::enable_if_t<
        ...
        std::is_same_v<typename ExampleBucketType::LabelBucketType,
                       LabelUnweightedBinaryCategoricalOneValueBucket>>> {
  using HWY_T = double;
  // Sorted with Key: Feature (double). Value: Label (bool, in lowest bit).
  static constexpr bool kIsSupported = true;
  static HWY_T* GetBuffer(PerThreadCacheV2* cache) {
    return cache->hwy_double_buffer.data();
  }
  ...
  static void Pack(HWY_T& hwy, const ExampleBucketType& bucket) {
    hwy =
        internal::DoublePayload::Set(static_cast<double>(bucket.feature.value),
                                     bucket.label.content.value ? 1 : 0);
  }
  static void Unpack(const HWY_T& hwy, ExampleBucketType& bucket) {
    bucket.feature.value =
        static_cast<float>(internal::DoublePayload::Clear(hwy));
    bucket.label.content.value = internal::DoublePayload::Get(hwy) != 0;
  }
};
