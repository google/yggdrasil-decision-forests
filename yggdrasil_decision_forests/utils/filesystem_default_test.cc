...
bazel build \
  //yggdrasil_decision_forests/utils:filesystem_default_test \
  --define=ydf_filesystem_support_gcs=1 && \
bazel-bin/yggdrasil_decision_forests/utils/filesystem_default_test \
  --alsologtostderr
*/
...
TEST(Filesystem, FileExistsAndDelete) {
  ...
  EXPECT_FALSE(FileExists(path).value());
  ...
  EXPECT_TRUE(FileExists(path).value());
  EXPECT_OK(RecursivelyDelete(path, file::Defaults()));
  EXPECT_FALSE(FileExists(path).value());
}
