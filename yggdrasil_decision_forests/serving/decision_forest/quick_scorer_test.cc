...
TEST(QuickScorer, Compilation) {
  ...
  GradientBoostedTreesRegressionQuickScorer quick_scorer_model;
...
TEST(QuickScorer, ExampleSet) {
  ...
  GradientBoostedTreesRegressionQuickScorer::ExampleSet examples(
      5, quick_scorer_model);
  examples.FillMissing(quick_scorer_model);

  EXPECT_EQ(quick_scorer_model.features().input_features().size(), 4);

  const auto feature_1 = GradientBoostedTreesRegressionQuickScorer::ExampleSet::
                             GetNumericalFeatureId("b", quick_scorer_model)
                                 .value();
  const auto feature_2 = GradientBoostedTreesRegressionQuickScorer::ExampleSet::
                             GetCategoricalFeatureId("c", quick_scorer_model)
                                 .value();
  const auto feature_3 = GradientBoostedTreesRegressionQuickScorer::ExampleSet::
                             GetCategoricalSetFeatureId("d", quick_scorer_model)
                                 .value();
  const auto feature_4 = GradientBoostedTreesRegressionQuickScorer::ExampleSet::
