...
using LeafMask = internal::QuickScorerModel::LeafMask;
...
absl::Status InitializeAccumulator(
    const AbstractModel& src, const internal::QuickScorerModel& dst,
    internal::QuickScorerModel::BuildingAccumulator* accumulator) {
    ...
    switch (feature.type) {
      case ColumnType::CATEGORICAL: {
        ...
        feature_acc.items.assign(
            ...
            ~internal::QuickScorerModel::kZeroLeafMask);
      } break;
...
absl::Status FinalizeModel(
    const internal::QuickScorerModel::BuildingAccumulator& accumulator,
    internal::QuickScorerModel* dst) {
  ...
  for (const auto& it_contains_condition :
    ...
    internal::QuickScorerModel::SparseContainsConditions condition;
    ...
    for (const auto& mask : src_masks) {
      ...
      for (const auto& tree_mask : mask) {
        if (tree_mask.second == ~internal::QuickScorerModel::kZeroLeafMask) {
          continue;
        }
...
absl::Status FillQuickScorerNode(
    ...
    const internal::QuickScorerModel::TreeIdx tree_idx,
    const NodeWithChildren& src_node, internal::QuickScorerModel* dst,
    int* leaf_idx, int* non_leaf_idx,
    internal::QuickScorerModel::BuildingAccumulator* accumulator) {
  if (src_node.IsLeaf()) {
    // Store the lead value.
    if (*leaf_idx >= internal::QuickScorerModel::kMaxLeafs) {
    ...
    const auto start_leaf_mask =
        (internal::QuickScorerModel::kOneLeafMask << begin_neg_leaf_idx) - 1;
    const auto after_neg_mask =
        (internal::QuickScorerModel::kOneLeafMask << end_neg_leaf_idx) - 1;
    internal::QuickScorerModel::LeafMask mask =
...
absl::Status FillQuickScorer(
    const AbstractModel& src, internal::QuickScorerModel* dst,
    internal::QuickScorerModel::BuildingAccumulator* accumulator) {
  ...
  if (dst->num_trees > internal::QuickScorerModel::kMaxTrees) {
    return absl::InvalidArgumentError(
        absl::Substitute("The model contains trees with more than $0 trees",
                         internal::QuickScorerModel::kMaxTrees));
  }
  ...
  if (dst->max_num_leafs_per_tree > internal::QuickScorerModel::kMaxLeafs) {
    return absl::InvalidArgumentError(
        absl::Substitute("The model contains trees with more than $0 leafs",
                         internal::QuickScorerModel::kMaxLeafs));
  }

  dst->leaf_values.assign(dst->max_num_leafs_per_tree * dst->num_trees, 0.f);

  for (internal::QuickScorerModel::TreeIdx tree_idx = 0;
...
void PredictQuickScorerSequential(
    ...
    internal::QuickScorerModel::LeafMask* active_leaf_buffer) {
...
template void PredictQuickScorer<GradientBoostedTreesRegressionQuickScorer>(
    const GradientBoostedTreesRegressionQuickScorer& model,
    const std::vector<NumericalOrCategoricalValue>& examples,
    const int num_examples, std::vector<float>* predictions);

template void Predict<GradientBoostedTreesRegressionQuickScorer>(
    const GradientBoostedTreesRegressionQuickScorer& model,
    const GradientBoostedTreesRegressionQuickScorer::ExampleSet& examples,
    const int num_examples, std::vector<float>* predictions);

template void Predict<GradientBoostedTreesRankingQuickScorer>(
    const GradientBoostedTreesRankingQuickScorer& model,
    const GradientBoostedTreesRankingQuickScorer::ExampleSet& examples,
    const int num_examples, std::vector<float>* predictions);

template <>
void Predict(
    const GradientBoostedTreesBinaryClassificationQuickScorer& model,
    const GradientBoostedTreesBinaryClassificationQuickScorer::ExampleSet&
        examples,
    const int num_examples, std::vector<float>* predictions) {
  PredictQuickScorerMajorFeatureOffset<
      GradientBoostedTreesBinaryClassificationQuickScorer,
...
absl::Status GenericToSpecializedModel(
    ...
    GradientBoostedTreesRegressionQuickScorer* dst) {
...
absl::Status GenericToSpecializedModel(
    const model::gradient_boosted_trees::GradientBoostedTreesModel& src,
    GradientBoostedTreesRankingQuickScorer* dst) {
...
absl::Status GenericToSpecializedModel(
    const model::gradient_boosted_trees::GradientBoostedTreesModel& src,
    GradientBoostedTreesBinaryClassificationQuickScorer* dst) {
...
CreateEmptyModel<GradientBoostedTreesRegressionQuickScorer>(
...
template std::string
DescribeQuickScorer<GradientBoostedTreesRegressionQuickScorer>(
    const GradientBoostedTreesRegressionQuickScorer& model, bool detailed);

template std::string
DescribeQuickScorer<GradientBoostedTreesBinaryClassificationQuickScorer>(
    const GradientBoostedTreesBinaryClassificationQuickScorer& model,
    bool detailed);
