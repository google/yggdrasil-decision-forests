...
class GradientBoostedTreesQuickScorerFastEngineFactory
 ...
 public:
  ...
  std::string name() const override {
    return serving::gradient_boosted_trees::kQuickScorer;
  }
  ...
  bool IsCompatible(const AbstractModel* const model) const override {
    ...
    if (gbt_model->NumTrees() >
        serving::decision_forest::internal::QuickScorerModel::kMaxTrees) {
      return false;
    }

    for (const auto& src_tree : gbt_model->decision_trees()) {
      if (src_tree->NumLeafs() >
          serving::decision_forest::internal::QuickScorerModel::kMaxLeafs) {
        return false;
      }
    }
  ...
  utils::StatusOr<std::unique_ptr<serving::FastEngine>> CreateEngine(
    ...
    switch (gbt_model->task()) {
      ...
      case proto::REGRESSION: {
        auto engine = absl::make_unique<serving::ExampleSetModelWrapper<
            serving::decision_forest::GradientBoostedTreesRegressionQuickScorer,
            serving::decision_forest::Predict>>();
        RETURN_IF_ERROR(engine->LoadModel<SourceModel>(*gbt_model));
        return engine;
      }
      ...
      case proto::RANKING: {
        auto engine = absl::make_unique<serving::ExampleSetModelWrapper<
            serving::decision_forest::GradientBoostedTreesRankingQuickScorer,
            serving::decision_forest::Predict>>();
        RETURN_IF_ERROR(engine->LoadModel<SourceModel>(*gbt_model));
        return engine;
      }
...
REGISTER_FastEngineFactory(GradientBoostedTreesQuickScorerFastEngineFactory,
                           serving::gradient_boosted_trees::kQuickScorer);
