...
//
// At its code, the algorithm manages a bitmap over the model leafs (called
...
// over the leafs if the condition is true. After all the condition have been
...
//
// Note: The algorithm is (or is in progress of being) patented
// (https://github.com/hpclab/quickscorer) and is only available by signing an
// NDA. This implementation was done using the published paper but without the
// quick scorer original source code.
//
...
#ifndef YGGDRASIL_DECISION_FORESTS_SERVING_DECISION_FOREST_QUICK_SCORER_H_
#define YGGDRASIL_DECISION_FORESTS_SERVING_DECISION_FOREST_QUICK_SCORER_H_
...
struct QuickScorerModel {
  using ExampleSet =
      ExampleSetNumericalOrCategoricalFlat<QuickScorerModel,
...
void AndMaskMap(const typename Map::key_type& key,
                QuickScorerModel::LeafMask mask, Map* map) {
...
struct GradientBoostedTreesRegressionQuickScorer : internal::QuickScorerModel {
...
struct GradientBoostedTreesBinaryClassificationQuickScorer
    : internal::QuickScorerModel {
...
struct GradientBoostedTreesRankingQuickScorer : internal::QuickScorerModel {
...
#endif  // YGGDRASIL_DECISION_FORESTS_SERVING_DECISION_FOREST_QUICK_SCORER_H_
