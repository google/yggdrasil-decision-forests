...
void PredictQuickScorerSequential(
    ...
    const std::vector<int32_t>& categorical_item_buffer, int begin_example_idx,
    int end_example_idx, int major_feature_offset,
...
#define HWY_TARGET_INCLUDE "yggdrasil_decision_forests/serving/decision_forest/quick_scorer_extended.cc"  // NOLINT
...
template <typename Model, float (*Activation)(float)>
void PredictQuickScorerMajorFeatureOffsetHighway(
    const Model& model,
    const std::vector<NumericalOrCategoricalValue>& fixed_length_features,
    const std::vector<Rangei32>& categorical_set_begins_and_ends,
  ...
  constexpr size_t alignment = sizeof(LeafMask) * kNumParallelExamples * 8;
...

  {
    ...
    while (num_remaining_iters--) {
      ...
      for (const auto& contains_condition :
        ...
        hn::StoreU(feature_values_vec, di, feature_vals);

        const LeafMask* leaf_mask_streams[kNumParallelExamples];
        for (size_t sub_example_idx = 0; sub_example_idx < kNumParallelExamples;
             ++sub_example_idx) {
        ...
        for (int tree_idx = 0; tree_idx < model.num_trees; ++tree_idx) {
          ...
          const auto leaf_masks = hn::LoadU(d, tmp_masks);
          auto* active_ptr =
              &active_leaf_buffer[tree_idx * kNumParallelExamples];
          const auto current_active = hn::LoadU(d, active_ptr);
...
void PredictQuickScorerMajorFeatureOffsetHighwayIdentity(
    const Model& model,
    const std::vector<NumericalOrCategoricalValue>& fixed_length_features,
    const std::vector<Rangei32>& categorical_set_begins_and_ends,
    const std::vector<int32_t>& categorical_item_buffer, const int num_examples,
    const int major_feature_offset, std::vector<float>* predictions) {
  PredictQuickScorerMajorFeatureOffsetHighway<Model,
...
template <typename Model>
void PredictQuickScorerMajorFeatureOffsetHighwayLogLikelihood(
    const Model& model,
    const std::vector<NumericalOrCategoricalValue>& fixed_length_features,
    const std::vector<Rangei32>& categorical_set_begins_and_ends,
    const std::vector<int32_t>& categorical_item_buffer, const int num_examples,
    const int major_feature_offset, std::vector<float>* predictions) {
  PredictQuickScorerMajorFeatureOffsetHighway<
      Model, internal::ActivationBinomialLogLikelihood>(
      model, fixed_length_features, categorical_set_begins_and_ends,
      categorical_item_buffer, num_examples, major_feature_offset, predictions);
}

template <typename Model>
void PredictQuickScorerMajorFeatureOffsetHighwayPoisson(
    const Model& model,
    const std::vector<NumericalOrCategoricalValue>& fixed_length_features,
    const std::vector<Rangei32>& categorical_set_begins_and_ends,
    const std::vector<int32_t>& categorical_item_buffer, const int num_examples,
  ...
  if constexpr (std::is_base_of_v<QuickScorerExtendedModelLegacy, Model>) {
    ...
    if constexpr (Activation == internal::ActivationBinomialLogLikelihood) {
      HWY_EXPORT_AND_DYNAMIC_DISPATCH_T(
          PredictQuickScorerMajorFeatureOffsetHighwayLogLikelihood<Model>)(
          model, fixed_length_features, categorical_set_begins_and_ends,
          categorical_item_buffer, num_examples, major_feature_offset,
          predictions);
    } else if constexpr (Activation == internal::ActivationPoisson) {
      HWY_EXPORT_AND_DYNAMIC_DISPATCH_T(
          PredictQuickScorerMajorFeatureOffsetHighwayPoisson<Model>)(
          model, fixed_length_features, categorical_set_begins_and_ends,
          categorical_item_buffer, num_examples, major_feature_offset,
          predictions);
    } else if constexpr (Activation == internal::ActivationIdentity) {
      HWY_EXPORT_AND_DYNAMIC_DISPATCH_T(
          PredictQuickScorerMajorFeatureOffsetHighwayIdentity<Model>)(
          model, fixed_length_features, categorical_set_begins_and_ends,
          categorical_item_buffer, num_examples, major_feature_offset,
          predictions);
    } else {
...
#else
    PredictQuickScorerMajorFeatureOffsetHighway<Model, Activation>(
        model, fixed_length_features, categorical_set_begins_and_ends,
        categorical_item_buffer, num_examples, major_feature_offset,
        predictions);
#endif  // YDF_USE_DYNAMIC_DISPATCH
...
ABSL_ATTRIBUTE_ALWAYS_INLINE float ActivationBinomialLogLikelihood(
    const float value) {
  return std::clamp(1.f / (1.f + std::exp(-value)), 0.f, 1.f);
}

ABSL_ATTRIBUTE_ALWAYS_INLINE float ActivationPoisson(const float value) {
  return std::exp(
      std::clamp(value,
...
ABSL_ATTRIBUTE_ALWAYS_INLINE float ActivationIdentity(const float value) {
  return value;
}
