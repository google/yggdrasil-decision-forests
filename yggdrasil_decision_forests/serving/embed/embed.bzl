...
def c_ydf_standalone_model(
        ...
        algorithm = "ROUTING",
        **attrs):
    """Embed a YDF model into a C library. Warning: This is not a C++ library!
    ...
    Args:
        ...
        classification_output: What is the Predict function is returning in case of a classification
            ...
            about those values. Has no impact on non-classification models.
        algorithm: How the predictions are computed. One of the values of
            "yggdrasil_decision_forests.serving.embed.proto.Algorithm". Not all options may be
            supported.
        **attrs: Classical cc_library attributes.
    """
    ...
    options = "classification_output: " + classification_output + " algorithm: " + algorithm + " c: {}"
    ...
    native.genrule(
        ...
        cmd = "$(location //yggdrasil_decision_forests/serving/embed:write_embed) --input=$$(cat $(location " + name + "_create_path ) ) --options='" + options + "' --output='$(RULEDIR)' --name=" + name + " --alsologtostderr",
        tools = ["//yggdrasil_decision_forests/serving/embed:write_embed", name + "_create_path", data],
    )

    # We link the .c file (srcs) and expose the .h file (hdrs).
    # This compels Bazel to compile the implementation logic into the library archive.
    cc_library(
        name = name,
        srcs = [name + ".c"],
        hdrs = [name + ".h"],
