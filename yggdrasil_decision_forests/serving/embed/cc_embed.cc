        ...
        if (feature_def.na_replacement.has_value()) {
          if (feature_def.type == "Numerical") {
            ...
            absl::SubstituteAndAppend(
                ...
                "  instance.$0 = instance.$0 < -2 ? 0 : instance.$0;\n"
                "  instance.$0 = instance.$0 >= $2 ? 0 : instance.$0;\n",
                feature_def.variable_name,    // $0
                *feature_def.na_replacement,  // $1
            ...
            return absl::InternalError("Invailid na_replacement value");
          }
        }
      }
    }
    ...
    case dataset::proto::ColumnType::CATEGORICAL: {
      if (col.categorical().is_already_integerized()) {
        return FeatureDef{
            ...
            .maximum_value =
                std::to_string(col.categorical().number_of_unique_values())};
      } else {
        return FeatureDef{
            .variable_name = StringToVariableSymbol(col.name()),
...
absl::Status GenerateTreeInferenceIfElseNode(
  ...
  const auto categorical_contains_condition =
    ...
    if (dataspec.columns(attribute_idx)
            .categorical()
      ...
      if (elements.size() < 8) {
        ...
        for (int element_idx = 0; element_idx < elements.size();
          ...
          const auto element_str = elements[element_idx];
        ...
        for (const auto element : elements) {
          ...
          absl::StrAppend(&mask, element);
        }

        // TODO: Don't hardcode uint32_t here.
        absl::SubstituteAndAppend(
            &condition,
            "std::array<Instance::IntegerizedCategorical,$0> mask = {$1};\n$3  "
            "  "
      ...
      if (elements.size() < 8) {
        ...
        std::string mask;
        for (const auto element : elements) {
          const auto element_str = cat_dict_it->second.sanitized_items[element];
          if (!mask.empty()) {
            absl::StrAppend(&mask, ",");
          }
