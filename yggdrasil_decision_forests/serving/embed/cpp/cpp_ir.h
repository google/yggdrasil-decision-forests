...
struct CppIR {
  ...
  NodeIdx num_trees = 0;
  std::string num_trees_type;

  // The C++ type for numerical values (float or some integer type). Will be
  // specified as a typedef.
  std::string numerical_type;
  // The C++ type for signed integer values.
  std::string signed_integer_type;
  // The C++ type for unsigned integer values.
  std::string unsigned_integer_type;

  // The custom enums required for categorical string features.
  std::vector<CppEnum> enums;
  std::vector<CppFeature> features;
  bool has_integerized_numerical_features = false;

  // --- Inference Engine Shared ---
  std::string accumulator_type;  // e.g., "std::array<float, 3>"
  ...
  // =========================================================================
  // The type for the offset to the next node.
  std::string node_offset_type;
  ...
  // Type used for the condition_types array.
  std::string condition_types_type;
  ...
  // Type used for the root_deltas array
  std::string root_deltas_type;
  // Comma-separated list of jump offsets to the root of each tree.
  std::string root_deltas_content;

  // C++ Type used for the respective union fields inside the Node struct.
  std::string pos_type;
  std::string feature_idx_type;
  std::string leaf_val_type;

  // Categorical mask variables. Empty if no categorical conditions are used.
  std::string cat_idx_type;
