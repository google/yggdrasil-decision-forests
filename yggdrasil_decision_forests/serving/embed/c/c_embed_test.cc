...
struct GoldenGeneratedCCase {
  std::string model_filename;
  std::string golden_header_filename;
...
SIMPLE_PARAMETERIZED_TEST(GoldenGeneratedC, GoldenGeneratedCCase,
  ...
  EXPECT_TRUE(embed.contains("ydf_model.c"));

  test::ExpectEqualGolden(
      embed.at("ydf_model.h"),
      file::JoinPath("yggdrasil_decision_forests/test_data/"
                     "golden/embed/c",
                     test_case.golden_header_filename));

  test::ExpectEqualGolden(
      embed.at("ydf_model.c"),
      file::JoinPath("yggdrasil_decision_forests/test_data/"
                     "golden/embed/c",
                     test_case.golden_implementation_filename));
}

// TODO : b/481596783 - Add additional unit tests.
