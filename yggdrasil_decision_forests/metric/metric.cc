  ...
  switch (src.calibration_case()) {
    case metric::proto::EvaluationResults::Classification::
      ...
      if (s.reliability_diagram_count().size() > nbins ||
        ...
        return absl::InvalidArgumentError(
            "Too many bins in calibration data during merging.");
      }
    ...
    default:
      return absl::InvalidArgumentError(absl::StrCat(
          "Unknown calibration data type: ", src.calibration_case()));
  }
  return absl::OkStatus();
}
...
absl::Status MergeEvaluationClassification(
  ...
  if (src.has_binary_calibration_data()) {
    RETURN_IF_ERROR(MergeCalibration(option, src, dst));
  }
  return absl::OkStatus();
}
...
ErrorBars<float> ComputeErrorIntervalForReliabilityDiagram(
  ...
  for (int i = 0; i < mean_predicted.size(); ++i) {
    ...
    if (alpha <= 0 || beta <= 0) {  // Prevents numerical rounding issues.
  ...
  switch (option.task()) {
    case model::proto::Task::CLASSIFICATION: {
      ...
      if (num_classes == 3 &&
        ...
        LOG(INFO)
            << "Classification is binary, but max_calibration_bins is not set. "
               "Calibration data will not be computed.";
      }
    } break;
    case model::proto::Task::REGRESSION:
